...
#REMOTE="${REMOTE:-https://github.com/glockmane/qubes-proxy/blob/main}"
REMOTE="${REMOTE:-https://raw.githubusercontent.com/glockmane/qubes-proxy/refs/heads/main}"
WORKDIR="/tmp"
...
echo 'nft add rule ip qubes custom-input iifname singtun0 ip saddr 10.139.1.0/30 accept' | sudo tee -a /rw/config/rc.local
echo 'nft add rule qubes custom-forward oifname eth0 counter drop' | sudo tee -a /rw/config/rc.local
echo 'nft add rule ip6 qubes custom-forward oifname eth0 counter drop' | sudo tee -a /rw/config/rc.local
echo 'nft add rule ip qubes custom-forward tcp flags syn / syn,rst tcp option maxseg size set rt mtu' | sudo tee -a /rw/config/rc.local
